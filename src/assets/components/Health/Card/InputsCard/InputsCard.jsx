...
const InputsCard = ({ bgIcon, icon, color, barColor, tittle, unit, minValue, midValue, maxValue, endpoint }) => {
    ...
    return (
      <form
        className="p-3 w-80 h-cardHeight bg-cards-background flex flex-col justify-center items-center gap-2
        rounded-lg shadow-lg shadow-black"
        onSubmit={handleSubmit(onSubmit)}
      >
        <div className="px-2 w-full flex flex-row justify-left items-center gap-4">
          <div
            className={`p-2 w-1/4 ${bgIcon} rounded-full flex flex-row justify-center items-center`}
          >
          ...
          <div className="w-3/4 flex flex-col justify-center items-left gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
              ...
              />
              <label htmlFor="values" className="text-lg text-white">
                {unit}
              </label>
            </div>
          </div>
        ...
        <div className="px-3 w-full flex flex-col gap-1">
          ...
          <div className="px-1 w-full flex flex-row justify-center gap-1">
            ...
            <div className={`h-5 grow rounded-l-lg ${bgIcon} overflow-hidden`}>
              {/* BARRA DE COLOR */}
              <div
                style={{ width: `${(valueForMin / minValue) * 100}%` }}
            ...
            <div className={`h-5 grow ${bgIcon} overflow-hidden`}>
              <div
                style={{
                  width: `${(valueForMid / (midValue - minValue)) * 100}%`,
                }}
                className={`h-full ${barColor}`}
              />
            </div>
            ...
            <div className={`h-5 rounded-r-lg grow ${bgIcon} overflow-hidden`}>
              <div
                style={{
                  width: `${(valueForMax / (maxValue - midValue)) * 100}%`,
                }}
                className={`h-full ${barColor}`}
              />
            </div>
        ...
        <div>
          <input
            type="submit"
            className=" font-Montserrat py-2 px-4 border border-transparent text-lg font-medium rounded-lg text-white bg-background hover:bg-backgound"
            value={"Añadir"}
          />
        </div>
      </form>
